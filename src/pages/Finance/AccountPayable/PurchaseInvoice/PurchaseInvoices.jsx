...
export default function PurchaseInvoices() {
  ...
  // Updated Post handler with confirmation
...
const handlePostInvoice = async (invoiceNo) => {
  try {
    const response = await fetch(`${import.meta.env.VITE_APP_FIN_URL}/api/values/PostPurchaseInvoice/${invoiceNo}`, {
      method: "POST",
    ...
    if (response.ok) {
      ...
      Swal.fire("Error!", errorData.message || "Failed to post invoice.", "error");
    }
  } catch (err) {
    ...
    <div className="bg-white px-4 py-8 rounded-lg relative">
      ...
      <div className="flex justify-between items-center mb-6 bg-indigo-800 px-6 py-3 rounded-2xl">
        ...
        <div className="flex gap-2">
          <Button className="bg-indigo-600 hover:bg-indigo-700 flex items-center gap-2" onClick={() => setShowAddVoucherDrawer(true)}>
            <FaPlus /> Add Voucher
          </Button>
          <Button className="bg-indigo-600 hover:bg-indigo-700 flex items-center gap-2" onClick={() => setShowAddDrawer(true)}>
            <FaPlus /> Add Invoice
          </Button>
        </div>
            ...
            {invoices.map((invoice, index) => (
              <div key={index} className="bg-white rounded-lg shadow-lg border">
                ...
                <div className="grid grid-cols-13 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  ...
                  <span className={`text-sm rounded-2xl text-center flex items-center justify-center px-3 py-1 col-span-2 mr-4 ${invoice.ApprovalStatus === "Approved" ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}>
                  ...
                  <div className="col-span-2 flex justify-end gap-2">
                    ...
                    >
                      {expandedInvoice === index? <><FaChevronUp /> Hide Lines</> : <><FaChevronDown /> View Lines</>}
                    </Button>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-40">
                        <DropdownMenuItem
                          ...
                          onClick={() => {
                            // Open update drawer with invoice
                ...
                {expandedInvoice === index && (
                  <div className="border-t bg-gray-200 p-3 rounded-b-lg">
                    {invoice.PurchaseInvoiceLines?.length > 0 ? (
                      ...
                      <p className="text-gray-500 text-sm italic px-6 py-3">No invoice lines found.</p>
                    )}
                  </div>
                )}
              </div>
      ...
      {/* Add & Update Drawers */}
      <AddVoucherDrawer open={showAddVoucherDrawer} onClose={() => setShowAddVoucherDrawer(false)} onSuccess={fetchInvoices} />
      <AddPurchaseInvoiceDrawer open={showAddDrawer} onClose={() => setShowAddDrawer(false)} onSuccess={fetchInvoices} />
      <UpdatePurchaseInvoiceDrawer open={showUpdateDrawer} onClose={() => setShowUpdateDrawer(false)} invoice={selectedInvoice} onSuccess={fetchInvoices} />
    </div>
  );
}
